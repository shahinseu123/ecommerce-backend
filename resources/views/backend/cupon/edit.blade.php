...
@section('content')
    <section>
        @if ($cupon)
            <form action="{{ route('backend.cupon.update') }}" method="POST">
                @csrf
                <div class="bg-white shadow-md rounded p-5">
                    <h2 class="text-2xl uppercase text-gray-600 font-semibold pb-3 border-b-2 border-gray-200">edit coupon
                    </h2>
                    <div class="mt-2">
                    ...
                    <div class="md:flex lg:flex xl:flex">
                        <div class="w-full md:w-1/3 lg:w-1/3 xl:w-1/3">
                            <ul class="border-r-2 border-gray-300">
                                <li class="cursor-pointer ac_active cu_link link_one py-3 pl-5 border-b-2  border-gray-300">
                                    <i class="fas fa-arrows-alt  mr-2"></i>General
                                </li>
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="mt-1">
                                        <input required type="number" name="discount_amount" id="discount_amount"
                                            ...
                                            value="{{ $cupon->discount_amount }}">
                                    </div>
                                </div>
                                <div class="mb-2 mt-3">
                                    <input type="checkbox" name="free_shiping"
                                        {{ $cupon->free_shiping == 'on' ? 'checked' : '' }} id="free_shiping"> <span>Check
                                        if
                                        the cupon
                                        allow
                                        free shipping</span>
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="mt-1">
                                        <input type="number" name="max_amount" id="max_amount"
                                            class="px-2 py-1 border-2 border-gray-300 rounded w-full w__max"
                                            value="{{ $cupon->max_amount }}">
                                    </div>
                                </div>
                                <div class="mb-2 mt-3">
                                    <input type="checkbox" {{ $cupon->individual_use_only == 'on' ? 'checked' : '' }}
                                        name="individual_use_only" id="individual_use_only"> <span>Check
                                        the
                                        box if the coupon can not be used in conjunction with other cupon.</span>
                                </div>
                                <div class="mb-2 mt-3">
                                    <input type="checkbox" {{ $cupon->exclude_sale_item == 'on' ? 'checked' : '' }}
                                        name="exclude_sale_item" id="exclude_sale_item"> <span>Check the
                                        box
                                        if the coupon should not apply product on sell.Per items cupon will work if the
                                        product
                                        not on sale</span>
                                </div>
                                <div class="mt-2">
                                    ...
                                    <select class="mt-4" name="include_product[]" id="ex-search" multiple>
                                        ...
                                        @if ($products)
                                            @foreach ($products as $item)
                                                <option
                                                    {{ in_array($item['id'], $cupon->Product->pluck('id')->toArray()) ? 'selected' : '' }}
                                ...
                                <div class="mt-2">
                                    ...
                                    <select class="mt-4" name="product_exclude[]" id="ex-search-2" multiple>
                                        ...
                                        @if ($products)
                                            @foreach ($products as $item)
                                                <option
                                                    {{ in_array($item['id'], $cupon->ProductEX->pluck('id')->toArray()) ? 'selected' : '' }}
                                                    value="{{ $item['id'] }}">{{ $item['title'] }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    ...
                                    <select class="mt-4" name="select_category[]" id="ex-search-3" multiple>
                                        ...
                                        @if ($categories)
                                            @foreach ($categories as $item)
                                                <option
                                                    {{ in_array($item['id'], $cupon->Category->pluck('id')->toArray()) ? 'selected' : '' }}
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="mt-1">
                                        <input type="number" name="cupon_usage_item_limit" id="cupon_usage_item_limit"
                                            class="px-2 py-1 border-2 border-gray-300 rounded w-full w__max"
                                            value="{{ $cupon->cupon_usage_item_limit }}">
                                    </div>
                                </div>
                                <div class="mb-2">
                                    ...
                                    <div class="mt-1">
                                        <input type="number" name="usage_limit_per_user" id="usage_limit_per_user"
                                            class="px-2 py-1 border-2 border-gray-300 rounded w-full w__max"
                                            value="{{ $cupon->usage_limit_per_user }}">
                                    </div>
                                </div>
                            </div>
                        </div>
